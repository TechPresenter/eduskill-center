...
export default async function StatesPage({ searchParams }: { searchParams: Promise<SearchParamsRecord> }) {
  ...
  return (
    <div>
      ...
      <Pagination className="mt-4" page={data.meta.page} totalPages={data.meta.totalPages} total={data.meta.total} limit={data.meta.limit} hrefFor={pageHref("/admin/states", sp)} />
    </div>
  );
}
