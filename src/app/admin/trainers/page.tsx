...
export default async function TrainersPage({ searchParams }: { searchParams: Promise<RawSearchParams> }) {
  ...
  return (
    ...
    >
      {all === 0 ? (
        <EmptyState icon={<UsersRound className="h-7 w-7" />} title="No trainers yet" description="A trainer record is created automatically when a volunteer application is approved." action={<ButtonLink href="/admin/trainer-applications" variant="outline">Review trainer applications</ButtonLink>} />
      ) : (
        <TableWrap>
          <THead>
            <tr>
            ...
            {data.items.map((t) => (
              <TR key={t.id}>
                ...
                <TD label="Level & location" className="max-md:text-right">
                  ...
                  <span className="mt-1 block text-caption text-muted">{[t.block?.name, t.district?.name, t.state.name].filter(Boolean).join(", ")}</span>
                </TD>
                <TD label="Assignments" className="md:max-w-[18rem]">
                  {t.assignments.length === 0 ? (
                    <span className="text-caption text-muted">Not assigned</span>
                  ) : (
                    <ul className="space-y-0.5 text-caption max-md:text-right">
