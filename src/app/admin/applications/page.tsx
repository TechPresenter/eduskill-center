...
export default async function ApplicationsPage({ searchParams }: { searchParams: Promise<RawSearchParams> }) {
  ...
  return (
    ...
    >
      {all === 0 ? (
        <EmptyState icon={<ClipboardList className="h-7 w-7" />} title="No applications yet" description="Applications students submit from the website and the student portal arrive here for review." />
      ) : (
        <TableWrap>
          <THead>
            <tr>
