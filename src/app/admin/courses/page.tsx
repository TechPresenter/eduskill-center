...
export default async function CoursesPage({ searchParams }: { searchParams: Promise<SearchParamsRecord> }) {
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          <>
            ...
            {canCreate && (
              <ButtonLink href="/admin/courses/new" size="sm" className="hidden sm:inline-flex" leftIcon={<Plus className="h-4 w-4" />}>
                Add course
              </ButtonLink>
            )}
          </>
      ...
      {canCreate && <Fab href="/admin/courses/new" aria-label="Add course" icon={<Plus className="h-6 w-6" />} className="sm:hidden" />}
      ...
      {data.meta.total === 0 && !filtered ? (
        ...
        <>
              ...
              {data.items.map((c) => (
                <TR key={c.id}>
                    ...
                    <span className="flex items-start justify-between gap-2">
                      <Link href={`/admin/courses/${c.id}`} className="flex min-w-0 items-center gap-3 hover:text-navy">
                        ...
                        <span className="min-w-0">
                          ...
                          <span className="block font-mono text-xs font-normal text-muted">{c.code}</span>
                        </span>
                      </Link>
                  ...
                  <TD label="Total fee" className="text-right tabular-nums">
                    {c.courseFee + c.registrationFee + c.examFee + c.certificateFee === 0 ? <span className="text-green-700">Free</span> : formatINR(c.courseFee + c.registrationFee + c.examFee + c.certificateFee)}
                  </TD>
          ...
          <Pagination className="mt-4" page={data.meta.page} totalPages={data.meta.totalPages} total={data.meta.total} limit={data.meta.limit} hrefFor={pageHref("/admin/courses", sp)} />
        </>
      )}
    </div>
  );
