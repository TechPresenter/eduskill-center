...
export default async function PaymentsPage({ searchParams }: { searchParams: Promise<RawSearchParams> }) {
  ...
  return (
    <div>
      <PageHeader title="Payments" description={`${formatNumber(data.meta.total)} payment${data.meta.total === 1 ? "" : "s"} in the current view · ${formatINR(data.totalCompleted)} completed.`} actions={<ExportButton href={withParams("/api/admin/payments/export", sp, { page: undefined, limit: undefined })} disabled={!can.export} />} />
      ...
      {kpis.all === 0 && !Object.keys(sp).length ? (
        <EmptyState icon={<CreditCard className="h-7 w-7" />} title="No payments yet" description="Payments appear when students pay fees or staff record them on an application." />
      ) : (
        <>
          <TableWrap>
            <THead>
              ...
              {data.items.map((p) => {
                ...
                return (
                  <TR key={p.id}>
                    <TD primary>
                      <span className="flex items-start justify-between gap-2">
                        <span className="min-w-0">
                          <Link href={`${base}/${p.id}`} className="font-mono text-xs font-semibold text-navy hover:underline">
                            {p.paymentNo}
                          </Link>
                          <span className="block text-[11px] font-normal text-muted">{p.receiptNo ? `Receipt ${p.receiptNo}` : `Invoice ${p.invoiceNo}`}</span>
                          {p.referenceNo && <span className="block text-[11px] font-normal text-muted">Ref {p.referenceNo}</span>}
                        </span>
                        {/* Amount + status lead the card on phones (those columns are dropped there). */}
                        <span className="shrink-0 text-right md:hidden">
                          <span className="block text-base font-bold tabular-nums text-navy">{formatINR(p.amount)}</span>
                          <StatusBadge status={p.status} className="mt-1" />
                        </span>
                      </span>
                      {p.failureReason && <span className="mt-1 block text-[11px] font-normal text-danger md:hidden">{p.failureReason}</span>}
                    </TD>
                    <TD label="Student">
                      <Link href={`/admin/students/${p.student.id}`} className="font-semibold hover:text-navy">
                        {p.student.name}
                      </Link>
                      <span className="block font-mono text-xs text-muted">{p.student.studentId ?? p.student.mobile}</span>
                    </TD>
                    <TD label="Application">
                      <Link href={`/admin/applications/${p.application.id}`} className="font-mono text-xs text-navy hover:underline">
                        {p.application.applicationNo}
                      </Link>
                      <span className="block text-xs text-muted">{p.application.course.name}</span>
                      <span className="block text-[11px] text-muted">{p.application.center.name}</span>
                    </TD>
                    ...
                    <TD label="Method">
                      {titleCase(p.method)}
                      <span className="block text-[11px] text-muted">{p.gateway}</span>
                    </TD>
                    <TD mobile="hidden">
                      <StatusBadge status={p.status} />
                      {p.failureReason && <span className="block max-w-48 truncate text-[11px] text-danger">{p.failureReason}</span>}
                    </TD>
