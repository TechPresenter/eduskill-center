  ...
  return (
    <>
      ...
      <div className="space-y-4 animate-fade-up motion-reduce:animate-none lg:hidden">
        ...
        {welcome === "1" ? (
          <Alert tone="success" title="Your account is ready" className="p-3 text-body-sm">
          ...
          <Alert tone="warning" title="Complete your profile" className="p-3 text-body-sm">
        ...
        {currentAdmission && classDays.length > 0 && (
          <section className="card p-4" aria-label="Classes this week">
            <p className="text-body-sm font-bold text-navy">Classes this week</p>
            <ul className="snap-row mt-2">
              {classDays.map((d) => (
                <li key={d.iso} className="rounded-md bg-lavender px-3 py-2 text-caption font-semibold whitespace-nowrap text-navy">
                  {d.label}
                </li>
              ))}
            </ul>
            <Link href="/student/timetable" className="mt-2 inline-flex min-h-11 items-center text-body-sm font-semibold text-orange">
              Full timetable
            </Link>
          </section>
        )}
        ...
        <section aria-label="Recent updates">
          <div className="mb-2 flex items-baseline justify-between gap-3 px-1">
            ...
            <Link href="/student/notifications" className="inline-flex min-h-11 items-center text-body-sm font-semibold text-orange">
              See all
            </Link>
          </div>
          {notifications.length === 0 ? (
            <div className="card">
              <EmptyState
                ...
                description="Updates about your applications, payments and classes arrive here."
              />
            </div>
          ) : (
            <ul className="card divide-y divide-line overflow-hidden">
              {notifications.slice(0, 3).map((n) => (
                <li key={n.id} className={n.readAt ? "px-4 py-3" : "bg-orange-light/25 px-4 py-3"}>
                  {/* An unread item is not signalled by the tint alone — it carries a visible marker too. */}
                  <p className="text-body flex items-start gap-2 font-semibold text-ink">
                    {!n.readAt && <span className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-orange" aria-label="Unread" />}
                    <span className="min-w-0">{n.title}</span>
                  </p>
                  <p className="text-body-sm mt-0.5 line-clamp-2 text-muted">{n.body}</p>
                  <p className="text-caption mt-1 text-muted">{formatDateTime(n.createdAt)}</p>
                </li>
              ))}
            </ul>
          )}
      ...
      <div className="hidden space-y-6 lg:block">
          ...
          <div className="bg-navy px-5 py-6 text-white sm:px-8">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-overline text-white/60">Student</p>
                <h2 className="truncate text-h2 text-white">{student.name}</h2>
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
                ...
                {currentAdmission ? (
                  <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
                    <div className="space-y-4 md:col-span-2">
                      <div>
                        <p className="text-caption font-semibold tracking-wide text-muted uppercase">Course</p>
                        <p className="text-body font-bold text-navy">{currentAdmission.course.name}</p>
                      </div>
                      <div>
                        <p className="text-caption font-semibold tracking-wide text-muted uppercase">Training center</p>
                        <p className="font-semibold text-ink">
                          {currentAdmission.center.name} <span className="font-mono text-caption text-muted">({currentAdmission.center.code})</span>
                        </p>
                        <p className="text-body-sm text-muted">{currentAdmission.center.address}</p>
                        {currentAdmission.center.phone && (
                          <a href={`tel:${currentAdmission.center.phone}`} className="mt-0.5 inline-flex items-center gap-1 text-body-sm text-navy hover:underline">
                      ...
                      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                          <p className="text-caption font-semibold tracking-wide text-muted uppercase">Batch</p>
                          <p className="font-semibold text-ink">
                            {currentAdmission.batch.name} <span className="font-mono text-caption text-muted">({currentAdmission.batch.code})</span>
                          </p>
                          <p className="text-body-sm text-muted">{formatSchedule(currentAdmission.batch)}</p>
                          {currentAdmission.batch.room && <p className="text-body-sm text-muted">Room: {currentAdmission.batch.room}</p>}
                          <p className="text-caption text-muted">
                            {formatDate(currentAdmission.batch.startDate)} – {formatDate(currentAdmission.batch.endDate)}
                          </p>
                        </div>
                        <div>
                          <p className="text-caption font-semibold tracking-wide text-muted uppercase">Trainer</p>
                          <p className="font-semibold text-ink">{currentAdmission.batch.trainer?.user.name ?? "To be assigned"}</p>
                          <p className="mt-3 text-caption font-semibold tracking-wide text-muted uppercase">Classes this week</p>
                          {classDays.length ? (
                            <ul className="mt-1 flex flex-wrap gap-1.5">
                              {classDays.map((d) => (
                                <li key={d.iso} className="rounded-md bg-lavender px-2 py-1 text-caption font-semibold text-navy">
                                  {d.label}
                                </li>
                              ))}
                            </ul>
                          ) : (
                            <p className="text-body-sm text-muted">No more classes this week</p>
                          )}
                        </div>
                      </div>
                    </div>
                    <div className="flex flex-col items-center justify-center gap-2 rounded-md bg-surface p-4">
                    ...
                    return (
                      <li key={a.id} className="flex flex-col gap-3 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                        <div className="min-w-0">
                          ...
                          <p className="mt-0.5 truncate text-body-sm text-ink">
                            {a.course.name} · {a.center.name}
                          </p>
                          <p className="text-caption text-muted">Applied {formatDate(a.createdAt)}</p>
                        </div>
          ...
          <Card className="self-start">
            ...
            {notifications.length === 0 ? (
              <CardBody>
                <EmptyState bare size="sm" icon={<Bell className="h-6 w-6" />} title="Nothing new yet" description="Updates about your applications, payments and classes arrive here." />
              </CardBody>
            ) : (
              <ul className="divide-y divide-line">
                {notifications.slice(0, 6).map((n) => (
                  <li key={n.id} className={`px-5 py-3 ${n.readAt ? "" : "bg-orange-light/30"}`}>
                    <p className="text-body-sm flex items-start gap-2 font-semibold text-ink">
                      {!n.readAt && <span className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-orange" aria-label="Unread" />}
                      <span className="min-w-0">{n.title}</span>
                    </p>
                    <p className="text-caption mt-0.5 line-clamp-2 text-muted">{n.body}</p>
                    <p className="text-caption mt-1 text-muted">{formatDateTime(n.createdAt)}</p>
                  </li>
                ))}
              </ul>
