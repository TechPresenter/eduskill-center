  ...
  return (
    <>
      <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        />
        <Button onClick={() => setOpen(true)} disabled={!canPost} leftIcon={<Plus className="h-4 w-4" />}>
          New announcement
        </Button>
      </div>
      ...
      {visible.length === 0 ? (
        <EmptyState
          ...
          description={announcements.length === 0 ? "Announcements from the Foundation and your centers will appear here. You can also post notices to your batches." : undefined}
          action={
            canPost && (
          ...
          {visible.map((a) => {
            ...
            return (
              <li key={a.id} className="card p-5">
                ...
                <p className="mt-3 text-sm whitespace-pre-wrap text-ink">{a.body}</p>
              </li>
            );
          })}
        </ul>
      ...
      <Drawer open={open} onClose={() => setOpen(false)} title="New batch announcement" description="Sent as an in-app notification to every active student of the batch." className="max-w-lg">
        {open && (
          <AnnouncementForm
            batches={postable}
