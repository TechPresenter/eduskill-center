...
export function CertificateRowActions({ certificateId, certificateNo, status, canRevoke }: { certificateId: string; certificateNo: string; status: string; canRevoke: boolean }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      <a href={`/api/admin/certificates/${certificateId}/download?download=1`} className="inline-flex h-8 items-center gap-1 rounded-lg border border-line bg-white px-2.5 text-xs font-semibold text-ink hover:bg-surface" aria-label={`Download certificate ${certificateNo}`}>
